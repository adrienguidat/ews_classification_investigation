...
df_ktau_forced = pd.read_csv('test_pitch/data/ews/df_ktau_forced.csv')   
df_ktau_null = pd.read_csv('test_pitch/data/ews/df_ktau_null.csv')

# Import ML prediction data
df_ml_forced = pd.read_csv('test_pitch/ml_preds/df_ml_forced.csv')
df_ml_null = pd.read_csv('test_pitch/ml_preds/df_ml_null.csv')
...
df_ml_preds['fav_bif'] = df_ml_preds[['fold_prob','hopf_prob','trans_prob','null_prob','pitch_prob']].idxmax(axis=1)
...
branch_count = counts['trans_prob'] if 'trans_prob' in counts.index else 0
null_count = counts['null_prob'] if 'null_prob' in counts.index else 0
pf_count= counts['pitch_prob'] if 'pitch_prob' in counts.index else 0
...
filepath = 'test_pitch/roc/df_bif_pred_counts_{}.csv'.format(
...
# Initialise list for ROC dataframes for predicting Rotating Hoop pitchfork bifurcation
list_roc = []
...
# Export ROC data
filepath = 'test_pitch/roc/df_roc_cr_hopf_{}.csv'.format(
    'early' if bool_pred_early else 'late')
...
fig.update_layout(
    ...
    width=600,
    height=600,
    title='ROC for Rotating Hoop simulations',
    )

fig.write_image('test_pitch/roc/roc_{}.png'.format('early' if bool_pred_early else 'late'))
